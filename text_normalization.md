...
* 特殊情况：完全不考虑全角/半角
  ...
  * 引号：如果前后有空格，则保留，如果没有，也不自动添加，但也有例外：
    * 左引号后面如果不是标点，则不需要空格
    * 右引号前面如果不是标点，则不需要空格
  * 加号和减号：因为有可能是正数和负数的表示，所以允许前面有空格，但前提是后面跟着的是数字
...
括号前加空格是因为括号的使用很多时候意味着语义的“分支”，使用空格隔离一下并不会太影响阅读。而且英文括号一般都很窄，不隔离会显得字符连在一起，特别是像下面的示例中的小写字母 l 之后的空格，如果 l 和括号之间没有空格，贴在一起影响阅读。英文括号太窄，中文括号太宽，就没有一个好看点的括号么？
